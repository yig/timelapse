...
def usage():
    print('Usage:', sys.argv[0], '[--glob] [--max-GB 2] path/to/image1 [path/to/image2 ... path/to/imageN] path/to/output_basename', file=sys.stderr)
    sys.exit(-1)
...
if len( output_shape ) != 1:
    print("Multiple output image shapes (read from the second-to-last _NxM_ part of the filename) detected.", file=sys.stderr)
    usage()
...
if max_images_in_memory < 1:
    ...
    print('(which would take', prod( output_shape ), 'bytes).', file=sys.stderr)
    usage()

stack = empty( ( output_shape[0]*output_shape[1], images_in_memory, output_shape[2] ), dtype = uint8 )
count = 0
for mem_off in range( 0, num_output_images, images_in_memory ):
    ...
    for save_off in range( 0, mem_end-mem_off ):
